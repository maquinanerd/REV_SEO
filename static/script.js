...
class SEODashboard {
    ...
    init() {
        ...
        // Countdown timer
        this.startCountdown();
        console.log('⏰ Contador regressivo ativado');
    }
    ...
    async loadStatistics() {
        try {
            ...
            if (result.success) {
                this.updateStatistics(result.data);
                this.updateChart(result.data.daily_stats);
                if (result.data.last_processing) {
                    this.lastProcessingTime = new Date(result.data.last_processing);
                }
            } else {
                throw new Error(result.error);
            }
    ...
    stopAutoRefresh() {
     ...
     startCountdown() {
        ...
        this.updateCountdown(); // Executa imediatamente
    }
    ...
    updateCountdown() {
        ...
        if (this.lastProcessingTime) {
            // Se temos o último processamento, adiciona 20 minutos
            nextProcessing = new Date(this.lastProcessingTime);
            nextProcessing.setMinutes(nextProcessing.getMinutes() + 20);
        } else {
            // Se não temos dados, assume próximo processamento em intervalos de 20 min
            const minutes = now.getMinutes();
            const nextInterval = Math.ceil(minutes / 20) * 20;
            nextProcessing = new Date(now);
            nextProcessing.setMinutes(nextInterval, 0, 0);

            // Se passou da hora, vai para a próxima hora
            if (nextInterval >= 60) {
                nextProcessing.setHours(nextProcessing.getHours() + 1);
                nextProcessing.setMinutes(0, 0, 0);
            }
        }

        const timeDiff = nextProcessing - now;
        ...
        if (timeDiff > 0) {
            ...
            const nextTimeElement = document.getElementById('nextProcessingTime');
            const countdownElement = document.getElementById('countdownTimer');
            const progressElement = document.getElementById('countdownProgress');

            if (nextTimeElement) {
                nextTimeElement.textContent = nextProcessing.toLocaleTimeString('pt-BR', {
                    hour: '2-digit',
                    minute: '2-digit'
                });
            }
            ...
            if (countdownElement) {
                ...
                countdownElement.className = 'badge ' + (minutes < 2 ? 'bg-danger' : minutes < 5 ? 'bg-warning' : 'bg-secondary');
            }

            if (progressElement) {
                // Progress bar (20 minutos = 100%)
                const totalMs = 20 * 60 * 1000; // 20 minutos em ms
                const progressPercent = Math.max(0, Math.min(100, ((totalMs - timeDiff) / totalMs) * 100));
                progressElement.style.width = progressPercent + '%';
            }
        } else {
            // Tempo esgotado, próximo processamento deve ser agora
            const nextTimeElement = document.getElementById('nextProcessingTime');
            const countdownElement = document.getElementById('countdownTimer');
            const progressElement = document.getElementById('countdownProgress');

            if (nextTimeElement) {
                nextTimeElement.textContent = 'Agora';
            }
    ...
    handleVisibilityChange() {
        ...
        if (this.isPageVisible) {
            ...
            this.startAutoRefresh();
             this.startCountdown();
            this.loadAllData(); // Carrega dados imediatamente
        } else {
            console.log('📱 Página oculta, pausando atualizações');
